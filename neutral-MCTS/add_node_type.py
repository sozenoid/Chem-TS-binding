...
def check_node_type(new_compound,SA_mean,SA_std,logP_mean,logP_std,cycle_mean,cycle_std):
    ...
    for i in range(len(new_compound)):
        ...
        if m!=None and len(new_compound[i])<=81:
            try:
                print("evaluation binding energy")
                # logp=Descriptors.MolLogP(m)
                logp, comp, guest=find_binding_affinity.return_binding_affinity(str(new_compound[i]))
                logp=-logp
                with open('./local_backup', 'at') as a:
                        print(logp, str(new_compound[i]))
                        a.write('{}\t{}\n'.format(logp, str(new_compound[i])))
            except Exception as e:
                logging.error(traceback.format_exc())
                logp=-1000
            if Chem.GetFormalCharge(m)>1: logp=-1000
            node_index.append(i)
    ...
    return node_index,score,valid_compound,all_smile, comp, guest
