...
def MCTS(root, verbose = False):
    ...
    """----------------------------------------------------------------------"""
    with open('./local_backup', 'wb') as w: pass
    w = Chem.SDWriter('./mols_and_complexes_found.sdf')
    while maxnum<100000:
        print(maxnum)
        node = rootnode
        state = root.Clone()
...
# =============================================================================
            ...
            node_index,score,valid_smile,all_smile, comp, guest=check_node_type(new_compound,SA_mean,SA_std,logP_mean,logP_std,cycle_mean,cycle_std)
            if not all([guest==None, comp==None]):
                w.write(guest)
                w.write(comp)
                w.flush()
            print(node_index)
    ...
    print("score, smi:",zip(all_score, valid_compound))
    print("depth=",depth)
    print(len(depth))
